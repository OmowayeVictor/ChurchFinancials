<template>
  <div class="p-6 bg-gray-100 min-h-screen">
    <div class="max-w-4xl mx-auto bg-white rounded-xl shadow p-6">
      ...
      >
        Editing Entry Session:
        <span class="font-bold">#{{ activeEntryId }}</span>
      </div>

      <form
        v-if="activeEntryId && openType"
      ...
      >
        ...
        >
          <label class="w-1/2 font-semibold text-gray-700">{{
            cat.category_name
          }}</label>
          <input
...
async function submitEdit() {
  ...
  if (!rows.length) {
    toast.add({
      severity: 'warn',
      summary: 'No Data',
      detail: `Enter at least one ${type.value} amount`,
      life: 3000,
    });
    loading.value = false;
    return;
  ...
  try {
    ...
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: err.message || 'Failed to update',
      life: 3000,
    });
  } finally {
    loading.value = false;
...
async function closeSession() {
  ...
  try {
    const column =
      openType.value === 'income' ? 'income_status' : 'expenses_status';
    ...
    toast.add({
      severity: 'success',
      summary: 'Closed',
      detail: `${openType.value} session closed`,
      life: 3000,
    });
    router.push('/entry');
  } catch (err) {
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: err.message || 'Failed to close session',
      life: 3000,
    });
  } finally {
    loading.value = false;
