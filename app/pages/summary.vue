        ...
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr
                v-for="(cat, index) in incomeSummary"
                :key="cat.category_id"
      ...
      <div class="mb-6">
        ...
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr
                v-for="(cat, index) in expensesSummary"
                :key="cat.category_id"
                class="hover:bg-gray-50 transition"
              >
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
          ...
          <div
            v-if="!loading && !expensesSummary.length"
            class="mt-2 text-gray-500 text-sm"
          >
...
const fetchSummary = async () => {
  ...
  try {
    ...
    totalIncome.value = incomeSummary.value.reduce(
    ...
    totalExpenses.value = expensesSummary.value.reduce(
      (acc, curr) => acc + curr.total,
      0
    );
    ...
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: err.message || 'Failed to fetch summary',
      life: 3000,
    });
  } finally {
    loading.value = false;
